...
function listenToQuota() {
    onSnapshot(REGISTRATIONS_COL, (snapshot) => {
        ...
        console.error("Kontenjan okuma hatası:", error);
    });
}
...
async function handleFormSubmit() {
    ...
    try {
        ...
        if (err.message === "KONTENJAN_DOLU") {
            ...
            notify.error("Erişim yetkisi hatası! Lütfen Firestore kurallarınızı kontrol edin.");
        } else {
            notify.error("Kayıt oluşturulurken bir hata meydana geldi: " + (err.message || "Bilinmeyen hata"));
        }
    }
}
